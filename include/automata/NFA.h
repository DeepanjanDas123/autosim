...
public:
    ...
    using Transition = std::unordered_map<TransitionKey, StateSet, TransitionKeyHash>;

    NFA(const StateSet& states,
        const std::unordered_set<Symbol>& alphabet,
        const Transition& transition,
        const State& startState,
        const StateSet& acceptStates);

    virtual bool accepts(const std::string& input) const;
...
public:
    EpsilonNFA(const StateSet& states,
               const std::unordered_set<Symbol>& alphabet,
               const Transition& transition,
               const State& startState,
               const StateSet& acceptStates);
